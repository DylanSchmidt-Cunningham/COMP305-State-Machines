...
public class BoidR : MonoBehaviour {
    ...
    public GameObject manager; // FlockR instance
    ...
    */
    ...
    void applyForce(Vector2 f)
    {
        ...
        if(force.magnitude > manager.GetComponent<FlockR>().maxForce)
        {
            force = force.normalized;
            force *= manager.GetComponent<FlockR>().maxForce;
        }
        this.GetComponent<Rigidbody2D>().AddForce(force);

        if(this.GetComponent<Rigidbody2D>().velocity.magnitude > manager.GetComponent<FlockR>().maxVelocity)
        {
            this.GetComponent<Rigidbody2D>().velocity = this.GetComponent<Rigidbody2D>().velocity.normalized;
            this.GetComponent<Rigidbody2D>().velocity *= manager.GetComponent<FlockR>().maxVelocity;
        }
    ...
    Vector2 align()
    {
        float neighbourDist = manager.GetComponent<FlockR>().neighbourDistance;
        Vector2 sum = Vector2.zero;
        int count = 0;
        foreach (GameObject other in manager.GetComponent<FlockR>().allBoids)
        {
            if (other == this.gameObject) continue;

            float d = Vector2.Distance(location, other.GetComponent<BoidR>().location);

            if (d < neighbourDist)
            {
                sum += other.GetComponent<BoidR>().velocity;
                count++;
            }
        }
    ...
    Vector2 cohesion()
    {
        ...
        foreach (GameObject other in manager.GetComponent<FlockR>().allBoids)
        {
            ...
            if (d < neighbourDist)
            {
                sum += other.GetComponent<BoidR>().location;
                count++;
            }
        }
    ...
    void flock()
    {
        ...
        if(manager.GetComponent<FlockR>().obedient && Random.Range(0,50)<= 1)
        {
            Vector2 ali = align();
            Vector2 coh = cohesion();
            Vector2 gl;
            if (manager.GetComponent<FlockR>().seekGoal)
            {
                gl = seek(goalPos);
        ...
        if(manager.GetComponent<FlockR>().willful && Random.Range(0,50) <= 1)
        {
    ...
    void ApplyRules()
    {
        ...
        Vector2 goalPos = FlockR.goalPos;
        ...
        foreach (GameObject go in gos)
        {
            if(go != this.gameObject)
            {
                ...
                if(dist <= neighbourDistance)
                {
                    ...
                    BoidR anotherBoidR = go.GetComponent<BoidR>();
                    gSpeed += anotherBoidR.speed;
                }
            }
        }
